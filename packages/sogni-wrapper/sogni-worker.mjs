...
async function runMultiAngleFlow(client, log) {
  ...
  const angleResults = [];
  const resultsMap = new Map(); // Store results by azimuth key to preserve order

  // Helper to process a single angle
  const processAngle = async (azimuth) => {
    ...
    const editConfig = {
      ...
      loras: options.loras.length > 0 ? options.loras : undefined,
      loraStrengths: options.loraStrengths.length > 0 ? options.loraStrengths : undefined,
      seed: options.seed
    };

    // Clean up undefined props
    Object.keys(editConfig).forEach(k => editConfig[k] === undefined && delete editConfig[k]);

    const { results } = await runImageEditProjectWithEvents(
      client,
      editConfig,
      options.count,
      (msg) => console.error(`[PROGRESS] Generating ${azimuth}: ${msg}`), // Custom log prefix
      options.timeout,
      azimuth
    );
  ...
  // Run in batches of 2
  const BATCH_SIZE = 2;
  for (let i = 0; i < azimuths.length; i += BATCH_SIZE) {
    const batch = azimuths.slice(i, i + BATCH_SIZE);
    console.error(`[PROGRESS] Starting batch ${i / BATCH_SIZE + 1}/${Math.ceil(azimuths.length / BATCH_SIZE)}: ${batch.join(', ')}`);

    // Process batch in parallel
    const batchResults = await Promise.all(batch.map(azimuth => processAngle(azimuth)));

    // Store results
    batchResults.forEach(result => {
      resultsMap.set(result.azimuth, result);
    });
  }

  // Reconstruct arrays in correct order
  const videoFrames = [];
  for (const azimuth of azimuths) {
    const result = resultsMap.get(azimuth);
    if (result) {
      angleResults.push(result);
      if (options.angles360Video && result.localPaths && result.localPaths.length > 0) {
        // Use the first image of each angle for the video
        videoFrames.push(result.localPaths[0]);
      }
    }
  }
